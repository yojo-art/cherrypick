...
@Injectable()
export class FullIndexReactionProcessorService {
	constructor(
	...
	public async process(_job: Bull.Job<Record<string, unknown>>): Promise<void> {
		await this.advancedSearchService.fullIndexReaction(FULL_INDEX_MAX_DURATION_MIN);

		const raw = await this.redisClient.get('fullIndexReaction:progress');
		if (!raw) return;
		...
		if (parsed.status === 'paused') {
			...
			await this.queueService.systemQueue.add('fullIndexReaction', {}, {
				delay: delayMs,
				jobId: `fullIndexReaction-${Date.now()}`,
			});
			await this.redisClient.set('fullIndexReaction:nextDelay', String(nextRunAt), 'EX', 600);
		}
	}
}
