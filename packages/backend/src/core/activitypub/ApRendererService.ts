...
export class ApRendererService {
	...
	public async renderAnnounce(object: string | IObject, note: MiNote): Promise<IAnnounce> {
		...
		//yojo-art: チャンネル連合 チャンネル投稿は宛先にチャンネルアカウントのフォロワーを含める（followers/specifiedは除外して漏洩防止）
		const channelFollowersUri = (note.channelId && note.userId && note.visibility !== 'followers' && note.visibility !== 'specified') ? await this.getChannelFollowersUri(note as MiNote & { channelId: string }) : null;
		if (channelFollowersUri && !to.includes(channelFollowersUri) && !cc.includes(channelFollowersUri)) {
			cc.push(channelFollowersUri);
		}
		const channelActorUri = (note.channelId && note.userId && note.visibility !== 'followers' && note.visibility !== 'specified') ? await this.getChannelUri(note as MiNote & { channelId: string }) : null;
	...
	public async renderNote(note: MiNote, dive = true): Promise<IPost> {
		...
		if (!isPureRenote(note) && note.channel?.actorId && note.visibility !== 'followers' && note.visibility !== 'specified')mentionUserIds.push(note.channel.actorId);
		...
		if (!isPureRenote(note) && note.channel?.actorId !== note.userId && note.visibility !== 'followers' && note.visibility !== 'specified') {
