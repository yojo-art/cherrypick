...
import { buildApEmojiUpdateFromTag, normalizeApEmojiTag } from '@/core/activitypub/misc/normalize-ap-emoji-tag.js';
...
describe(buildApEmojiUpdateFromTag, () => {
	it('部分タグでは指定されたフィールドだけを更新する', () => {
		const tag = createApEmojiTag({ copyPermission: 'deny' });
		const normalized = normalizeApEmojiTag(tag);

		expect(buildApEmojiUpdateFromTag(tag, normalized)).toEqual({
			copyPermission: 'deny',
		});
	});

	it('keywords が欠落しているとき aliases は更新対象に含めない', () => {
		const tag = createApEmojiTag({ copyPermission: 'deny' });
		const normalized = normalizeApEmojiTag(tag);

		expect(buildApEmojiUpdateFromTag(tag, normalized)).not.toHaveProperty('aliases');
	});

	it('keywords が明示されているとき aliases を更新する', () => {
		const tag = createApEmojiTag({ keywords: ['new', 'alias'] });
		const normalized = normalizeApEmojiTag(tag);
